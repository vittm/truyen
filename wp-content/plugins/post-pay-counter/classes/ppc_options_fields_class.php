...
class PPC_options_fields {
    ...
    static function echo_payment_systems( $counting_type, $counting_type_localized, $settings ) {
        ...
        while( $n < $zones_count ) {
            $html .= '<tr>';
            $html .= '<td><input type="text" size="15" name="'.$counting_type.'_'.$n.'_zone_threshold" id="'.$counting_type.'_'.$n.'_zone_threshold" value="'.$settings['counting_'.$counting_type.'_system_zonal_value'][$n]['threshold'].'" /></td>';
            $html .= '<td><input type="text" size="15" name="'.$counting_type.'_'.$n.'_zone_payment" id="'.$counting_type.'_'.$n.'_zone_payment" value="'.sprintf( '%.2f', $settings['counting_'.$counting_type.'_system_zonal_value'][$n]['payment'] ).'" /></td>';
            $html .= '</tr>';
            ++$n;
        }
