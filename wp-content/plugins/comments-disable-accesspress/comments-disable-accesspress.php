 ...
 * Description: A plugin to disable comments globally with various configurable options
 * Version: 1.0.5
...
if (!defined('CDAP_VERSION')) {
    define('CDAP_VERSION', '1.0.5'); // plugin version information
}
...
if (!class_exists('Comments_Disable_AP')) {
    ...
    class Comments_Disable_AP {
        ...
        function plugin_text_domain() {
            load_plugin_textdomain('ap-comments-disable', false, basename(dirname(__FILE__)) . '/languages/');
        }
        ...
        function plugin_option_page() {
            add_options_page(__('Comments Disable', 'ap-comments-disable'), __('Comments Disable', 'ap-comments-disable'), 'manage_options', 'comments-disable-ap', array($this, 'plugin_settings'));
        }
        ...
        function save_settings() {
            if ( !empty( $_POST ) && wp_verify_nonce( $_POST['cdap_settings_nonce'], 'cdap_settings_save') ) {
                ...
                update_option( 'cdap_settings', $cdap_settings );
                wp_redirect( admin_url() . 'options-general.php?page=comments-disable-ap&message=1' );
                exit;
            } else {
                die('No script kiddies please!');
            }
